...
import { ProbePanel, buildProbeInfo } from './probeMode.jsx';
...
const DataFlowDiagram = ({ content, revealed, onReveal, probeEnabled = false, onProbe }) => {
            ...
            return (
              <g key={node.id}>
                <rect
                  ...
                  onClick={() => { onReveal(node.id); if (probeEnabled && onProbe) onProbe('dataflow-node', node); }}
                  role="button"
...
const ConceptMap = ({ content, revealed, onReveal, probeEnabled = false, onProbe }) => {
  return (
    <div className="space-y-4">
      {content.description && (
        <p className="text-slate-300 text-sm">{content.description}</p>
        ...
        {content.branches.map(branch => {
          ...
          return (
            <div
              ...
              onClick={() => { onReveal(branch.id); if (probeEnabled && onProbe) onProbe('concept-branch', branch); }}
              role="button"
              aria-expanded={isRevealed}
...
const FaultTree = ({ content, revealed, onReveal, probeEnabled = false, onProbe }) => {
  const getIcon = (type) => {
    switch (type) {
      case 'error': return '❌';
...
const ModuleView = ({ module, category, progress, notes, onBack, onComplete, onSaveNote }) => {
  ...
  const [probeEnabled, setProbeEnabled] = useState(config.features?.probeMode ?? true);
  const [probeSelection, setProbeSelection] = useState(null);
  ...
  const handleProbeSelect = (elementType, element) => {
    const info = buildProbeInfo({ module, category, elementType, element });
    setProbeSelection({ elementType, element, info });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className={`p-2 rounded-lg transition-colors ${probeEnabled ? 'bg-violet-700 hover:bg-violet-600' : 'hover:bg-slate-700'}`}
          aria-label="Toggle probe mode"
          title="Probe mode (P)"
        >
      ...
      {probeEnabled && (
        <div className="space-y-2">
          <div className="text-xs text-slate-400">
            Probe Mode is on: click a node/branch to get a guided readout.
          </div>
          <ProbePanel selection={probeSelection} onClose={() => setProbeSelection(null)} />
        </div>
      )}
</div>
